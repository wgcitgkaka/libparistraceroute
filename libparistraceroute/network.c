...
static inline bool reply_extract_tag_ipid(const probe_t * reply, 
uint16_t * ptag_reply_ipid) {
    return probe_extract_ext(reply, "identification", 2, ptag_reply_ipid);
}
...
static probe_t * network_get_matching_probe(network_t * network, const probe_t * reply)
{
    ...
    uint16_t   tag_probe, tag_reply,tag_reply_ipid;
    probe_t  * probe;
   ...
   // Fetch the tag ipid from the reply. Its the 2nd ipid field.
    if (!(reply_extract_tag_ipid(reply, &tag_reply_ipid))) {
        // This is not an IP/ICMP/IP/* reply :(
        if (network->is_verbose) fprintf(stderr, "Can't retrieve tag ipid from reply\n");
        return NULL;
    }
    ...
    for (i = 0; i < num_flying_probes; i++) {
        ...
        if (probe_extract_tag(probe, &tag_probe)) {
            if (tag_reply == tag_probe || tag_reply_ipid == tag_probe) break;
        }
    }
...
bool network_tag_probe(network_t * network, probe_t * probe)
{
    ...
    //write the tag  in ipid if it is udp or tcp or icmp
   probe_write_field(probe, "identification", &tag, tag_size);
